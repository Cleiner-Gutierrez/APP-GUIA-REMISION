...
export default function UsuariosPage() {
  ...
  const usuarios = [
    ...
    { id: 2, nombre: "Ana García", email: "ana@empresa.com", rol: "Operador" },
  ];
    ...
    <div className="h-full w-full bg-gradient-to-br from-emerald-600 via-emerald-500 to-amber-400 p-4">
      <div className="flex flex-col h-full gap-4 animate-in fade-in duration-500">
        ...
        <div className="flex-none flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-black text-white">Usuarios del Sistema</h1>
            <p className="text-emerald-50 text-sm">Gestiona los accesos y privilegios del personal.</p>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {usuarios.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-bold text-slate-900">{user.nombre}</td>
