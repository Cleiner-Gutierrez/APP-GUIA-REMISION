...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-600 via-emerald-500 to-amber-400 p-8 flex items-center">
      <div className="flex flex-col gap-10 max-w-7xl w-full mx-auto">
        
        {/* Saludo */}
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-white tracking-tight">
            Bienvenido, <span className="text-emerald-50">{session?.user?.name || "Administrador"}</span>
          </h1>
          <p className="text-emerald-100/90 text-lg font-medium">Panel de control centralizado | Plastecniva S.A.S.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {cards.map((card) => (
            <Link 
              key={card.title}
              href={card.path}
              className="relative overflow-hidden bg-white/95 backdrop-blur-md p-8 rounded-3xl border border-white/20 shadow-2xl hover:shadow-[0_20px_50px_rgba(0,0,0,0.2)] transition-all duration-500 hover:-translate-y-2 group"
            >
              ...
              <div className="relative flex items-center gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-slate-500 font-semibold">{card.desc}</p>
                </div>
